    ...
    def __call__(self, img, cls=True):
        ...
        for bno in range(len(dt_boxes)):
            ...
            if self.args.det_box_type == "quad":
                ...
                shape1 = img_crop.shape[1]/img_crop.shape[0]
                new_width = int(shape1*48)
                img_crop = cv2.resize(img_crop,(new_width,48))
            else:
...
def sorted_boxes(dt_boxes):
    ...
    for i in range(num_boxes - 1):
        for j in range(i, -1, -1):
            if abs(_boxes[j + 1][0][1] - _boxes[j][0][1]) < 50 and (_boxes[j + 1][0][0] < _boxes[j][0][0]):
                tmp = _boxes[j]
                _boxes[j] = _boxes[j + 1]
                _boxes[j + 1] = tmp
            else:
